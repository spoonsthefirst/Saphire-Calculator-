...
window.configure(bg="#2c3e50")  # Set background color to blue
...
try:
    ...
    image = image.resize((20, 20))  # Resize the image to your desired size
    photo = ImageTk.PhotoImage(image)
    image_label = tk.Label(window, image=photo)
    image_label.image = photo  # Keep a reference to the image to avoid garbage collection
    image_label.grid(row=0, column=3, columnspan=4, pady=10, sticky="nsew")
